...
$Server->add_handler(ADD => {
	...
	call => sub {
		...
		my $cart = ALKO::Cart->Get(id_merchant => 1, n => 1)  or return $S->fail('OBJECT: Can\'t add Product to Cart: no such cart(id_merchant => 1, n => 1)');
...
$Server->add_handler(CART => {
	...
	call => sub {
		...
		my $cart = ALKO::Cart->Get(id_merchant => 1, n => 1) or return $S->fail('OBJECT: Can\'t get Cart: no such cart(id_merchant => 1, n => 1)');
...
$Server->add_handler(DELETE => {
	...
	call => sub {
		...
		my $cart = ALKO::Cart->Get(id_merchant => 1, n => 1) or return $S->fail('OBJECT: Can\'t delete Product from Cart: no such cart(id_merchant => 1, n => 1)');
