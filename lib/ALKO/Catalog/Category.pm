...
sub complete_products {
	...
	for my $prop (ALKO::Catalog::Property::Value->All(id_product => [$self->products->List('id')])->List) {
		...
		$id_country->{$prop->val_int}      = undef if $country_prop->id_propgroup      == $prop->id_propgroup and $country_prop->n      == $prop->n_property and $prop->val_int and int $prop->val_int;
		$id_brend->{$prop->val_int}        = undef if $brand_prop->id_propgroup        == $prop->id_propgroup and $brand_prop->n        == $prop->n_property and $prop->val_int and int $prop->val_int;
		$id_manufacturer->{$prop->val_int} = undef if $manufacturer_prop->id_propgroup == $prop->id_propgroup and $manufacturer_prop->n == $prop->n_property and $prop->val_int and int $prop->val_int;
